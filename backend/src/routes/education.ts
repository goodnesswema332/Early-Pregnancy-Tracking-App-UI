      ...
      data: [
        {
          ...
          answer:
            "Reproductive health education teaches you about your body, how it works, and how to make informed decisions about your health and future.",
        },
      ],
    });
  } catch (err: any) {
...
router.put("/faq/:id", protect, async (req: any, res) => {
  try {
    const f = await Faq.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    if (!f)
    ...
    data: [
      {
        ...
        explanation: "Once a girl starts menstruating, pregnancy is possible.",
      },
    ],
  });
});
...
router.put("/topics/:id", protect, async (req: any, res) => {
  try {
    const t = await Topic.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    if (!t)
      return res.status(404).json({ success: false, message: "Not found" });
