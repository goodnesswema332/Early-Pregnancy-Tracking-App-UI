...
router.post('/sessions', async (req: any, res) => {
  try {
    ...
    const session = await ChatSession.create({ userLabel: userLabel || `Anon-${Date.now()}` });
...
router.get('/sessions', protect, async (req: any, res) => {
  try {
    ...
    const sessions = await ChatSession.find(filter).sort({ createdAt: -1 }).populate('assignedTo', 'name email role');
...
router.get('/sessions/:id', protect, async (req: any, res) => {
  try {
    const session = await ChatSession.findById(req.params.id).populate('assignedTo', 'name email role');
    if (!session) return res.status(404).json({ success: false, message: 'Session not found' });
    const messages = await ChatMessage.find({ sessionId: session._id.toString() }).sort({ timestamp: 1 });
...
router.get('/sessions/:id/messages', async (req: any, res) => {
  try {
    const messages = await ChatMessage.find({ sessionId: req.params.id }).sort({ timestamp: 1 });
...
router.post('/sessions/:id/messages', async (req: any, res) => {
  try {
    ...
    const msg = await ChatMessage.create({ sessionId, message, sender: 'user' });
...
router.post('/sessions/:id/messages/admin', protect, async (req: any, res) => {
  try {
    const { message } = req.body;
    const sessionId = req.params.id;
    const msg = await ChatMessage.create({ sessionId, message, sender: 'support', userId: req.user._id });
    await ChatSession.findByIdAndUpdate(sessionId, { status: 'active', assignedTo: req.user._id });
    const io = req.app.get('io');
    if (io) {
      io.to(`session:${sessionId}`).emit('message:created', msg);
      io.to('admins').emit('session:message', { sessionId, message: msg });
...
router.put('/sessions/:id/assign', protect, async (req: any, res) => {
  try {
    ...
    const session = await ChatSession.findByIdAndUpdate(req.params.id, { assignedTo: adminId, status: 'assigned' }, { new: true }).populate('assignedTo', 'name email role');
    if (!session) return res.status(404).json({ success: false, message: 'Session not found' });
    const io = req.app.get('io');
...
router.put('/sessions/:id/close', protect, async (req: any, res) => {
  try {
    const session = await ChatSession.findByIdAndUpdate(req.params.id, { status: 'closed', closedAt: new Date(), assignedTo: null }, { new: true });
    if (!session) return res.status(404).json({ success: false, message: 'Session not found' });
    const io = req.app.get('io');
    if (io) io.to('admins').emit('session:updated', session);
    res.json({ success: true, data: session });
