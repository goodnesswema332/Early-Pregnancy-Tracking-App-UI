...
export const createEmbeddedVideo = async (req: any, res: Response) => {
  try {
    ...
    if (!title || !url)
      return res
        .status(400)
        .json({ success: false, message: "title and url required" });
    const vid = await EmbeddedVideo.create({
      title,
      url,
      provider,
      description,
...
export const updateEmbeddedVideo = async (req: any, res: Response) => {
  try {
    ...
    const upd = await EmbeddedVideo.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!upd)
...
export const listEmbeddedVideos = async (req: Request, res: Response) => {
  try {
    const videos = await EmbeddedVideo.find()
      .sort({ createdAt: -1 })
      .limit(100);
