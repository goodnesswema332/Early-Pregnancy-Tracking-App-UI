...
import { generateAccessToken, generateRefreshTokenPair } from "../utils/generateToken";
...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    console.log('authController.register: total users after create=', totalAfterCreate, 'collection=', (User as any).collection?.name, 'mongoose readyState=', mongoose.connection.readyState, 'dbName=', mongoose.connection.name || (mongoose.connection as any).db?.databaseName);
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    console.log('authController.login: total users in collection=', totalUsers, 'collection=', (User as any).collection?.name, 'mongoose readyState=', mongoose.connection.readyState, 'dbName=', mongoose.connection.name || (mongoose.connection as any).db?.databaseName);
    try {
      const docs = await (mongoose.connection.db as any).collection((User as any).collection?.name || 'users').find({}).toArray();
    ...
    console.log('authController.login: user found for', email, 'passwordPresent=', !!(user as any).password);
...
export const refreshToken = async (req: Request, res: Response) => {
  try {
    ...
    const parsed = (await import("../utils/generateToken")).parseRefreshToken(refreshToken);
    if (!parsed) return res.status(401).json({ success: false, message: "Invalid token format" });

    const { tokenId, tokenSecret } = parsed as any;
    const existing = await RefreshToken.findOne({ tokenId });
    if (!existing) return res.status(401).json({ success: false, message: "Invalid token" });
    ...
    if (!valid) return res.status(401).json({ success: false, message: "Invalid token" });

    // rotation: remove old and issue new
    await existing.deleteOne();
    const { token, tokenId: newId, tokenSecret: newSecret, expiresAt } = generateRefreshTokenPair() as any;
    const newHash = await bcrypt.hash(newSecret, 10);
    await RefreshToken.create({ user: existing.user, tokenId: newId, tokenHash: newHash, expiresAt });

    res.json({ success: true, data: { accessToken: generateAccessToken(existing.user.toString()), refreshToken: token } });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const seedAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (exists) {
      return res.json({ success: true, data: { _id: exists._id, email: exists.email, role: exists.role, name: exists.name, accessToken: generateAccessToken(exists._id.toString()) } });
    }
    ...
    return res.status(201).json({ success: true, data: { _id: user._id, name: user.name, email: user.email, role: user.role, accessToken: generateAccessToken(user._id.toString()), refreshToken: token } });
  } catch (error: any) {
