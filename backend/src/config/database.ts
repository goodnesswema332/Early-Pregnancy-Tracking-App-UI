...
const connectDB = async (): Promise<void> => {
  try {
    const conn = await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/pregnancy-prevention-app');
    ...
    try {
      ...
      if (seedEmail && seedPassword) {
        ...
        if (!exists) {
          const u = await User.create({ name: 'Seed Admin', email: seedEmail, password: seedPassword, role: seedRole as any });
